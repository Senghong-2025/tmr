    ...
    <div class="w-full relative">
        ...
        <div :class="[
            ...
            <input :type="type" v-model="model" :placeholder="placeholder" :disabled="disabled" @change="emit('change')" @keydown="emit('keydown')" :max="maxDate ?? ''" :min="minDate ?? ''"
                class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100" :inputmode="mode || 'text'"/>
        </div>
